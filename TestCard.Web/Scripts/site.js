...
function showError(errorText, errorTitle) {
    ...
    var $content = $('<span>').append($('<span/>').addClass('icon')).append(errorText);
...
function showSuccess(text, title) {
    ...
    var $content = $('<span>').append($('<span/>').addClass('icon')).append(text);

    popup.setContent($content);
    popup.addClass('x-success-window');
...
function registerAjaxForm($form, onSuccess, onError, overrideError) {
...
function initValidation() {
    $('.field-validation-error').each(function (index, elem) {
        ...
        var hasSpan = $t.find('span').length > 0;
